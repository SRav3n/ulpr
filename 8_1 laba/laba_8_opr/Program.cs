interface IVector
{
    abstract bool Kollin(vector3D other);
    abstract bool LengthComparison(vector3D other);
}
...
class vector3D : IVector
{
    ...
    {
        get { return Math.Sqrt(Math.Pow(x_axis, 2) + Math.Pow(y_axis, 2) + Math.Pow(z_axis, 2)); }
    }

    public bool Kollin (vector3D other) 
    {
    ...
    public bool LengthComparison (vector3D other)
    {
